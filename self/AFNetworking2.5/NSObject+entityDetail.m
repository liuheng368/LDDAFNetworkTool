...
-(NSString *)DsSetValueAndUndefinedKey:(NSDictionary *)KeyAndValue underfinedEntity:(NSString *)entity
{
    if (![KeyAndValue[@"key"] isEqualToString:@"seq_no"]) {
        NSLog(@"-----未完整定义键值%@ : %@， 对应实体：%@",KeyAndValue[@"key"],KeyAndValue[@"value"],entity);
    }
    //一般操作
    //    if ([key[@"key"] isEqualToString:@"id"]) {
